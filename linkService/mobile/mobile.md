...
1. PKCE 값 생성
2. `/mobile/authorize` 호출
3. 응답에서 `nonce`, `qrData`, 세션 쿠키 저장
4. `qrData` 로 BMC 앱 실행 (모바일공무원증앱 호출)
5. `/mobile/txCheck` 를 반복 호출해서 `COMPLETE` 확인
6. `/login` 호출 시 `username=nonce`, `password=nonce` 사용
7. `/login` 응답의 JSON 에서 `code` 추출
8. `/oauth2/token` 호출 시 `code`, 원래의 `code_verifier`, 동일한 `redirect_uri`, 클라이언트 인증 정보 사용
