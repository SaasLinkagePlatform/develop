...
GPKI 연계 API는 중앙 및 지방 행정 기관에 종사하는 공직자들이 민간 SaaS에 로그인 시, <br/>
...
- `redirect_uri`: GPKI 연계 API에 인가 코드를 요청하고 리다이렉트되는 민간 SaaS의 서비스 URI
...
인증을 위한 Access Token의 유효 기간은 1시간입니다. Access Token이 만료되면 발급받은 리프래쉬 토큰으로 갱신 요청을 하여 엑세스 토큰을 재발급받습니다.
